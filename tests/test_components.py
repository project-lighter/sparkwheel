...
- Item: Basic configuration item wrapper
- Component: Instantiable components with _target_
- Expression: Python expression evaluation
- Resolver: Reference and dependency resolution
...
class TestItem:
    ...
    def test_init(self):
        ...
        config = {"key": "value"}
        item = Item(config, id="test_id")
        assert item.config == config
        assert item.id == "test_id"

    def test_init_default_id(self):
        """Test Item with default empty id."""
        config = [1, 2, 3]
        item = Item(config)
        assert item.config == config
        assert item.id == ""
    ...
    def test_get_id(self):
        """Test get_id method."""
        item = Item({"a": 1}, id="my_id")
        assert item.get_id() == "my_id"

    def test_update_config(self):
        """Test update_config method."""
        item = Item({"a": 1}, id="test")
...
class TestComponent:
    ...
    def test_is_instantiable_false_not_dict(self, value):
        ...
        assert Component.is_instantiable(value) is False
...
class TestExpression:
    ...
    def test_is_expression_true(self, expr):
        ...
        assert Expression.is_expression(expr) is True
    ...
    def test_is_expression_false(self, value):
        ...
        assert Expression.is_expression(value) is False
    ...
    def test_is_import_statement_true(self, stmt):
        ...
        assert Expression.is_import_statement(stmt) is True
    ...
    def test_is_import_statement_false(self, value):
        ...
        assert Expression.is_import_statement(value) is False
    ...
    def test_evaluate_with_locals(self):
        ...
        expr = Expression("$local_var * 2", id="test")
    ...
    def test_evaluate_not_expression(self):
        ...
        expr = Expression("normal string", id="test")
        result = expr.evaluate()
        assert result is None
    ...
    def test_evaluate_import_as(self):
        ...
        expr = Expression("$import collections as col", id="test")
        result = expr.evaluate()
    ...
    def test_evaluate_builtin_functions(self):
        ...
        expr = Expression("$len([1, 2, 3, 4])", id="test")
        result = expr.evaluate()
        assert result == 4
    ...
    def test_is_import_statement_edge_cases(self):
        ...
        assert not Expression.is_import_statement("regular string")
        assert not Expression.is_import_statement("$1 + 1")
        assert not Expression.is_import_statement("$'import is a keyword'")
...
class TestResolver:
    ...
    def test_init_empty(self):
        """Test Resolver initialization with no items."""
        resolver = Resolver()
        assert resolver._items == {}
        assert resolver._resolved == {}

    def test_init_with_items(self):
        """Test Resolver initialization with items."""
        item1 = Item({"a": 1}, id="item1")
        item2 = Item({"b": 2}, id="item2")
        resolver = Resolver(items=[item1, item2])
        assert len(resolver._items) == 2
        assert "item1" in resolver._items
        assert "item2" in resolver._items

    def test_reset(self):
        """Test reset clears items and resolved content."""
        resolver = Resolver()
        resolver.add_item(Item({"a": 1}, id="test"))
        resolver._resolved["test"] = {"a": 1}
        assert len(resolver._items) > 0
        assert len(resolver._resolved) > 0
    ...
    def test_add_item(self):
        """Test add_item adds Item."""
        resolver = Resolver()
        item = Item({"a": 1}, id="test")
        resolver.add_item(item)
        assert "test" in resolver._items
        assert resolver._items["test"] is item

    def test_add_item_duplicate_ignored(self):
        """Test add_item ignores duplicate id."""
        resolver = Resolver()
        item1 = Item({"a": 1}, id="test")
        item2 = Item({"b": 2}, id="test")
        resolver.add_item(item1)
        resolver.add_item(item2)
        assert resolver._items["test"] is item1
        assert resolver._items["test"].config == {"a": 1}
    ...
    def test_get_item_with_resolve(self):
        ...
        assert retrieved is item
        assert "test" in resolver._resolved

    def test_normalize_id(self):
        """Test normalize_id converts to string."""
        assert Resolver.normalize_id("a::b::c") == "a::b::c"
        assert Resolver.normalize_id("simple") == "simple"
        assert Resolver.normalize_id(123) == "123"
    ...
    def test_update_refs_pattern_multiple(self):
        ...
        refs = {"a": 1, "b": 2}
        result = Resolver.update_refs_pattern("$@a + @b", refs)
    ...
    def test_update_refs_pattern_substring_handling(self):
        ...
        refs = {"a": 1, "ab": 2}
        result = Resolver.update_refs_pattern("$@ab + @a", refs)
    ...
    def test_update_refs_pattern_missing_ref_warning(self):
        ...
        original = Resolver.allow_missing_reference
        try:
            Resolver.allow_missing_reference = True
            refs = {}
            with warnings.catch_warnings(record=True) as w:
                warnings.simplefilter("always")
                Resolver.update_refs_pattern("@missing", refs)
                assert len(w) > 0
                assert "can not find" in str(w[0].message)
        finally:
            Resolver.allow_missing_reference = original

    def test_update_refs_pattern_missing_ref_error(self):
        """Test update_refs_pattern raises error on missing reference."""
        original = Resolver.allow_missing_reference
        try:
            Resolver.allow_missing_reference = False
            refs = {}
    ...
    def test_update_config_with_refs_string(self):
        ...
        refs = {"myref": 42}
        result = Resolver.update_config_with_refs("@myref", "test", refs)
        assert result == 42
    ...
    def test_update_config_with_refs_dict(self):
        ...
        result = Resolver.update_config_with_refs(config, "test", refs)
        assert isinstance(result, dict)
    ...
    def test_resolve_one_item_with_reference(self):
        ...
        resolver = Resolver()
        resolver.add_item(Item({"value": 10}, id="base"))
        resolver.add_item(Item({"ref": "@base"}, id="derived"))
    ...
    def test_resolve_one_item_circular_reference(self):
        ...
        from sparkwheel.utils.exceptions import CircularReferenceError

        resolver = Resolver()
        resolver.add_item(Item({"ref": "@b"}, id="a"))
        resolver.add_item(Item({"ref": "@a"}, id="b"))
    ...
    def test_resolve_one_item_missing_reference_error(self):
        ...
        resolver = Resolver()
        resolver.add_item(Item({"ref": "@missing"}, id="test"))
    ...
    def test_resolve_one_item_component(self):
        """Test _resolve_one_item with Component."""
        resolver = Resolver()
        config = {"_target_": "dict", "a": 1, "b": 2}
        component = Component(config, id="comp")
        resolver.add_item(component)
        result = resolver._resolve_one_item("comp")
        assert result == {"a": 1, "b": 2}

    def test_resolve_one_item_component_no_instantiate(self):
        """Test _resolve_one_item with Component and instantiate=False."""
        resolver = Resolver()
    ...
    def test_resolve_one_item_expression(self):
        """Test _resolve_one_item with Expression."""
        resolver = Resolver()
        expr = Expression("$1 + 1", id="expr")
        resolver.add_item(expr)
        result = resolver._resolve_one_item("expr")
        assert result == 2

    def test_resolve_one_item_expression_no_eval(self):
        """Test _resolve_one_item with Expression and eval_expr=False."""
        resolver = Resolver()
        expr = Expression("$1 + 1", id="expr")
        resolver.add_item(expr)
    ...
    def test_resolve_one_item_import_statement_priority(self):
        ...
        resolver = Resolver()
        resolver.add_item(Expression("$import os", id="imp"))
    ...
    def test_resolve_one_item_with_default(self):
        ...
        resolver = Resolver()
        default_item = Item({"default": True}, id="default")
    ...
    def test_resolve_one_item_with_non_config_item(self):
        """Test _resolve_one_item when stored item is not a Item."""
        resolver = Resolver()
        resolver.add_item(Item({"val": 1}, id="test"))
